...
Platform support:
- Claude Code: Full support via CLI eval
- OpenCode/OpenClaw: Requires Task tool (see run_eval_opencode.py)
"""
...
from scripts.platform import detect_platform, supports_cli_eval
...
def run_loop(
    ...
    Args:
        ...
        skill_path: Path to skill being optimized
        description_override: Override starting description
        ...
        holdout: Fraction to hold out for testing
        model: Model for improvement generation
    ...
    Returns:
        Dict with best description and history
    ...
    if not supports_cli_eval(platform):
        raise RuntimeError(
            f"Platform '{platform}' does not support CLI-based description optimization.\n"
            f"Use Task tool for manual testing instead:\n"
...
def main():
    ...
    if not supports_cli_eval(platform):
        print(
            f"Error: Platform '{platform}' does not support CLI-based description optimization.\n"
            f"Use Task tool for manual testing instead:\n"
            f"  python scripts/run_eval_opencode.py prepare --skill-path <path> --eval-set <json>\n",
            file=sys.stderr,
        )
    ...
    if args.report != "none":
        ...
        # Only open browser on Claude Code
